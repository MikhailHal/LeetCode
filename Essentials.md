...
### 気づき（Maximum Difference Between Node and Ancestor）
- グローバル変数でpath_max/path_minを持つと、別ブランチの値が混ざる
- DFSは「深く潜って戻る」ので、戻ったときにグローバル変数は元に戻らない
- 「このパス上の状態」を保ちたいなら、引数で持ち運ぶ

### 抽象
**「戻ったとき元に戻したい状態 → 引数、累積していい状態 → グローバル」**

- 引数で渡す → 各再帰呼び出しがスナップショットを持つ → 戻っても影響なし
- グローバル → 全体で共有 → 戻っても値はそのまま

### 使えると気づくトリガー
- **「このパス上の」max/min/sumを追跡したい** → 引数
- **「祖先-子孫の関係」を保証したい** → 引数
- **「答えの最大値を集計するだけ」** → グローバルでOK
- **「訪問済み管理」** → グローバルでOK（戻す必要がない）

### 目的・メリット
- 引数: パスごとに独立した状態を持てる、バグりにくい
- グローバル: コードがシンプルになる（適切な場面で使えば）

### 別具体
| 状態の種類 | 持ち方 | 理由 |
|-----------|--------|------|
| パス上のmax/min | 引数 | 戻ったとき元に戻したい |
| バックトラッキングの選択リスト | 引数 or 手動で戻す | 戻ったとき元に戻したい |
| 答えの集計（ans） | グローバル | 更新するだけ |
| 訪問済み（visited） | グローバル | 戻す必要なし |
